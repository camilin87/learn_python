...
def scale_down (dim, scale):
    ...
    if type(dim) in ( TupleType, ListType ):
        res=[]
        for x in dim:
            res.append(x/scale)
        return tuple(res)
...
class ArrowHeadCanvas( DragCanvas ):
    ...
    def draw_arrow_head_controls(self):
        ...
        text=`scale_down(minlen, self.scale)`,
        ...
        text=`scale_down(maxlen, self.scale)`
        ...
        text=`scale_down(height, self.scale)`       
        ...
        if self.outline:
            ...
            self.outline = apply( self.create_line, (coords,options) )
        ...
        if not self.shape_text:
            ...
            self.itemconfigure(self.shape_text, text=`shape` )
    ...
    def draw_arrow_width_controls(self):
        ...
        if  not self.width_arrow:
            ...
            self.width_arrow = apply( self.create_line,
                                      (coords, options)
                                    )
        else:
        ...
        options = {'text':`scale_down(self.arrow_width, self.scale)`,
                   'anchor':'e' }
        
        if not self.width_smalltext:
            self.width_smalltext = apply(
                self.create_text, (coords, options ) )
        else:
        ...
        if not self.width_text :
            ...
            self.itemconfigure(self.width_text, text=`w` )
